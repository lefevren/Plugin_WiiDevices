...
class CryVR_WiimoteBalanceBoardControlerPlugin : public CFlowBaseNode<eNCT_Instanced>
{
    public:
        ...
        ////////////////////////////////////////////////////
        CryVR_WiimoteBalanceBoardControlerPlugin( SActivationInfo* pActInfo ) :
            m_actInfo()
        {
        }
        ...
        ////////////////////////////////////////////////////
        virtual ~ CryVR_WiimoteBalanceBoardControlerPlugin( void ) {}
        ...
        virtual void ProcessEvent( EFlowEvent event, SActivationInfo* pActInfo )
        {
            ...
            if ( event == eFE_Update )
            {
                ...
                if ( CryVR_WiimoteManagerPlugin::GetWiimotes( w_id )->exp.type == EXP_WII_BOARD && was_motion_sensing_active )
                {
                ...
                //if(CryVR_WiimoteManagerPlugin::found >w_id && w_id>=0 && CryVR_WiimoteManagerPlugin::wiimotes[0]->exp.type == EXP_WII_BOARD) {
                while ( wiiuse_poll( CryVR_WiimoteManagerPlugin::GetWiimotes(), MAX_WIIMOTES ) && i < GetPortInt( pActInfo, 4 ) )
                {
                    i++;
                    ...
                    if ( CryVR_WiimoteManagerPlugin::GetWiimotes( w_id )->event == WIIUSE_EVENT )
                    {
                        ...
                        {
                            handle_event_calib( CryVR_WiimoteManagerPlugin::GetWiimotes( w_id ) );
                            nb_values_calib ++ ;
                        }

                        else
                        {
                            if ( GetPortBool( pActInfo, 5 ) && i > GetPortInt( pActInfo, 4 ) )
                            {
                                return ;
                            }
        ...
        {
            return new  CryVR_WiimoteBalanceBoardControlerPlugin( pActInfo );
        }
...
REGISTER_FLOW_NODE( "CryVR:Controlers:Wii_Plugin:BalanceBoardControler",  CryVR_WiimoteBalanceBoardControlerPlugin );
