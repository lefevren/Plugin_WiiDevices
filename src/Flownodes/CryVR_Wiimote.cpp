...
class CFlowWiimoteControlerPlugin : public CFlowBaseNode<eNCT_Instanced>
{
        enum EInputPorts
        {
            EIP_IsServer = 0,
    ...
    public:
        ...
        ////////////////////////////////////////////////////
        CFlowWiimoteControlerPlugin( SActivationInfo* pActInfo )
        {

        }

        ////////////////////////////////////////////////////
        virtual ~CFlowWiimoteControlerPlugin( void )
        {
                ...
                case eFE_Initialize:
                    {
                        ...
                        {
                            CryLogAlways( "Failed to connect to any wiimote.\n" );
                        }
                ...
                case eFE_Update:
                    {
                        ...
                        {
                            ActivateOutput( pActInfo, WIIMOTE_A, true );
                        }

                        else
                        {
                            ActivateOutput( pActInfo, WIIMOTE_A, false );
                        }

                        if ( IS_PRESSED( wiimotes[0], WIIMOTE_BUTTON_B ) )
                        {
                            ActivateOutput( pActInfo, WIIMOTE_B, true );
                        }

                        else
                        {
                            ActivateOutput( pActInfo, WIIMOTE_B, false );
                        }

                        if ( IS_PRESSED( wiimotes[0], WIIMOTE_BUTTON_UP ) )
                        {
                            ActivateOutput( pActInfo, WIIMOTE_UP, true );
                        }

                        else
                        {
                            ActivateOutput( pActInfo, WIIMOTE_UP, false );
                        }

                        if ( IS_PRESSED( wiimotes[0], WIIMOTE_BUTTON_DOWN ) )
                        {
                            ActivateOutput( pActInfo, WIIMOTE_DOWN, true );
                        }

                        else
                        {
                            ActivateOutput( pActInfo, WIIMOTE_DOWN, false );
                        }

                        if ( IS_PRESSED( wiimotes[0], WIIMOTE_BUTTON_LEFT ) )
                        {
                            ActivateOutput( pActInfo, WIIMOTE_LEFT, true );
                        }

                        else
                        {
                            ActivateOutput( pActInfo, WIIMOTE_LEFT, false );
                        }

                        if ( IS_PRESSED( wiimotes[0], WIIMOTE_BUTTON_RIGHT ) )
                        {
                            ActivateOutput( pActInfo, WIIMOTE_RIGHT, true );
                        }

                        else
                        {
                            ActivateOutput( pActInfo, WIIMOTE_RIGHT, false );
                        }

                        if ( IS_PRESSED( wiimotes[0], WIIMOTE_BUTTON_MINUS ) )
                        {
                            ActivateOutput( pActInfo, WIIMOTE_MINUS, true );
                        }

                        else
                        {
                            ActivateOutput( pActInfo, WIIMOTE_MINUS, false );
                        }

                        if ( IS_PRESSED( wiimotes[0], WIIMOTE_BUTTON_PLUS ) )
                        {
                            ActivateOutput( pActInfo, WIIMOTE_PLUS, true );
                        }

                        else
                        {
                            ActivateOutput( pActInfo, WIIMOTE_PLUS, false );
                        }

                        if ( IS_PRESSED( wiimotes[0], WIIMOTE_BUTTON_ONE ) )
                        {
                            ActivateOutput( pActInfo, WIIMOTE_ONE, true );
                        }

                        else
                        {
                            ActivateOutput( pActInfo, WIIMOTE_ONE, false );
                        }

                        if ( IS_PRESSED( wiimotes[0], WIIMOTE_BUTTON_TWO ) )
                        {
                            ActivateOutput( pActInfo, WIIMOTE_TWO, true );
                        }

                        else
                        {
                            ActivateOutput( pActInfo, WIIMOTE_TWO, false );
                        }

                        if ( IS_PRESSED( wiimotes[0], WIIMOTE_BUTTON_HOME ) )
                        {
                            ActivateOutput( pActInfo, WIIMOTE_HOME, true );
                        }

                        else
                        {
                            ActivateOutput( pActInfo, WIIMOTE_HOME, false );
                        }
                        ...
                        if ( wm->exp.type == EXP_NUNCHUK )
                        {
                            ...
                            {
                                ActivateOutput( pActInfo, NUNCHUK_C, true );
                            }

                            else
                            {
                                ActivateOutput( pActInfo, NUNCHUK_C, false );
                            }

                            if ( IS_PRESSED( nc, NUNCHUK_BUTTON_Z ) )
                            {
                                ActivateOutput( pActInfo, NUNCHUK_Z, true );
                            }

                            else
                            {
                                ActivateOutput( pActInfo, NUNCHUK_Z, false );
                            }
        ...
        {
            return new CFlowWiimoteControlerPlugin( pActInfo );
        }
        ...
        */
        void handle_event( struct wiimote_t* wm )
        {
            ...
            {
                CryLogAlways( "A pressed\n" );
            }

            if ( IS_PRESSED( wm, WIIMOTE_BUTTON_B ) )
            {
                CryLogAlways( "B pressed\n" );
            }

            if ( IS_PRESSED( wm, WIIMOTE_BUTTON_UP ) )
            {
                CryLogAlways( "UP pressed\n" );
            }

            if ( IS_PRESSED( wm, WIIMOTE_BUTTON_DOWN ) )
            {
                CryLogAlways( "DOWN pressed\n" );
            }

            if ( IS_PRESSED( wm, WIIMOTE_BUTTON_LEFT ) )
            {
                CryLogAlways( "LEFT pressed\n" );
            }

            if ( IS_PRESSED( wm, WIIMOTE_BUTTON_RIGHT ) )
            {
                CryLogAlways( "RIGHT pressed\n" );
            }

            if ( IS_PRESSED( wm, WIIMOTE_BUTTON_MINUS ) )
            {
                CryLogAlways( "MINUS pressed\n" );
            }

            if ( IS_PRESSED( wm, WIIMOTE_BUTTON_PLUS ) )
            {
                CryLogAlways( "PLUS pressed\n" );
            }

            if ( IS_PRESSED( wm, WIIMOTE_BUTTON_ONE ) )
            {
                CryLogAlways( "ONE pressed\n" );
            }

            if ( IS_PRESSED( wm, WIIMOTE_BUTTON_TWO ) )
            {
                CryLogAlways( "TWO pressed\n" );
            }

            if ( IS_PRESSED( wm, WIIMOTE_BUTTON_HOME ) )
            {
                CryLogAlways( "HOME pressed\n" );
            }
...
REGISTER_FLOW_NODE( "CryVR:Controlers:Wii_Plugin:ControlerOBSOLETE", CFlowWiimoteControlerPlugin );
