...
class CryVR_WiimoteGuitarHeroControlerPlugin : public CFlowBaseNode<eNCT_Instanced>
{
    public:
        ...
        ////////////////////////////////////////////////////
        CryVR_WiimoteGuitarHeroControlerPlugin( SActivationInfo* pActInfo ) :
            m_actInfo()
        {
        }
        ...
        ////////////////////////////////////////////////////
        virtual ~ CryVR_WiimoteGuitarHeroControlerPlugin( void ) {}
        ...
        virtual void ProcessEvent( EFlowEvent event, SActivationInfo* pActInfo )
        {
            ...
            if ( event == eFE_Update )
            {
                ...
                if ( !active || w_id < 0 || w_id >= CryVR_WiimoteManagerPlugin::found )
                {
                    return ;
                }
                ...
                if ( CryVR_WiimoteManagerPlugin::GetMotionSensing( w_id ) && CryVR_WiimoteManagerPlugin::GetWiimotes( w_id )->exp.type == 0 )
                {
                    was_motion_sensing_active = true;
                    CryLogAlways( " *** GuitarHero Controler force Motion Sensing off during connection *** " );
                    CryVR_WiimoteManagerPlugin::SetMotionSensing( w_id, false );
                }

                if ( CryVR_WiimoteManagerPlugin::GetWiimotes( w_id )->exp.type == EXP_CLASSIC && was_motion_sensing_active )
                {
                    CryLogAlways( " *** GuitarHero set Motion Sensing previous state *** " );
                    //CryVR_WiimoteManagerPlugin::SetMotionSensing(w_id,true);
                    was_motion_sensing_active = false;
                    CryVR_WiimoteManagerPlugin::Status( CryVR_WiimoteManagerPlugin::GetWiimotes( w_id ) );
                }


                /* fait planter la balance board ! */

                while ( wiiuse_poll( CryVR_WiimoteManagerPlugin::GetWiimotes(), CryVR_WiimoteManagerPlugin::found ) && i < GetPortInt( pActInfo, 2 ) )
                {
        ...
        {
            return new  CryVR_WiimoteGuitarHeroControlerPlugin( pActInfo );
        }
...
REGISTER_FLOW_NODE( "CryVR:Controlers:Wii_Plugin:GuitarHeroControler",  CryVR_WiimoteGuitarHeroControlerPlugin );
